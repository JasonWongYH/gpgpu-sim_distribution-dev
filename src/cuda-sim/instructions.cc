...
//1 May 2017 We need to fix the widths for data conversion between FPn for any n 
void cvt_impl(const ptx_instruction *pI, ptx_thread_info *thread) { 
  ...
  unsigned src_fmt = type_info_key::type_decode(from_type, from_width, from_sign); //TODO see list of values 
  ...
  if(pI->is_neg()){ //TODO when is this ever true?
    switch(from_type) {
        case S8_TYPE:
        case U8_TYPE:
        case B8_TYPE:
        ...
        case F32_TYPE:
          ...
          break;
        case F64_TYPE:
        case FF64_TYPE: //.ff64 is a data type recognized in ptx
          data.f64 = -data.f64;
          break;
        default:
          assert(0); //TODO what happens here
    }
  }
  //TODO find out what's this doing, fix then
...
//1 May 2017 Need to extend this for FPn for any n
void mad_def(const ptx_instruction *pI, ptx_thread_info *thread, bool use_carry) { 
  ...
  const operand_info &src3 = pI->src3();
   ...
   int carry=0;
   int overflow=0;
   unsigned i_type = pI->get_type(); //TODO instruction type I guess
   ptx_reg_t a = thread->get_operand_value(src1, dst, i_type, thread, 1);
   ptx_reg_t b = thread->get_operand_value(src2, dst, i_type, thread, 1);
   ptx_reg_t c = thread->get_operand_value(src3, dst, i_type, thread, 1);
   ...
   case S64_TYPE: 
      ...
      break;
   case U16_TYPE: //TODO Can I cover FP16_TYPE with U16_TYPE and S16_TYPE
      t.u32 = a.u16 * b.u16;
   ...
   case U64_TYPE: 
      ...
      break;
   case F16_TYPE: {
      //assert(0); //TODO 'monkey see monkey do'
         assert(use_carry == false); 
         int orig_rm = fegetround();
         switch (rounding_mode) {
           case RN_OPTION: break;
         ...
         d.f16 = a.f16 * b.f16 + c.f16;//TODO where's this set?
