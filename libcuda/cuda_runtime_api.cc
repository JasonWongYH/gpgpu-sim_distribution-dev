//*TODO cudaMallocManaged back be implemented by 1) calling CUDA malloc and initialize a function pointer to cudaMemcpy, and 2) when the kernel is launched, invoke the cudaMemCopy along with cudaLaunch.*/
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <assert.h>
...
//TODO need to call API expecting *fatCubinHandle Check if fatCubin structure is preserved 
...
__host__ cudaError_t cudaMallocManaged(
  ...
  void** devPtr,
  size_t         size,
  unsigned int   flags = 1//cudaMemAttachGlobal
){
