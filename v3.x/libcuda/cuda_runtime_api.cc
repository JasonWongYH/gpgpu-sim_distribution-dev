...
#include "../src/gpgpu-sim/memory_owner.h"
...
__host__ cudaError_t CUDARTAPI cudaMemcpy(void *dst, const void *src, size_t count, enum cudaMemcpyKind kind)
{
	...
	if( kind == cudaMemcpyHostToDevice )
		...
		assert(false && "GPGPU-Sim PTX: cudaMemcpy - ERROR : unsupported cudaMemcpyKind");
	}
	return g_last_cudaError = cudaSuccess;
}
...
__host__ cudaError_t CUDARTAPI cudaMemcpy2D(void *dst, size_t dpitch, const void *src, size_t spitch, size_t width, size_t height, enum cudaMemcpyKind kind)
{
	...
	if( kind == cudaMemcpyHostToDevice )
		...
		assert(false && "GPGPU-Sim PTX: cudaMemcpy2D - ERROR : unsupported cudaMemcpyKind");
	}
	return g_last_cudaError = cudaSuccess;
}
...
__host__ cudaError_t CUDARTAPI cudaLaunch(const char *hostFun) {
  ...
  assert(stream);
  kernel_info_t *grid = gpgpu_cuda_ptx_sim_init_grid(hostFun, config.get_args(), config.grid_dim(),
	...
	for ( ; g!=symtab->global_iterator_end(); g++) {
		...
		if ( global->has_initializer() ) {
			...
			for ( std::list<operand_info>::iterator i=init_list.begin(); i!=init_list.end(); i++ ) {
				...
				gpu->get_global_memory()->write(addr+offset,nbytes,&value,NULL,NULL);
				offset+=nbytes;
				ng_bytes+=nbytes;
			}
		}
	...
	for ( ; g!=symtab->const_iterator_end(); g++) {
		...
		if ( constant->is_const() && constant->has_initializer() ) {
			...
			for ( std::list<operand_info>::iterator i=init_list.begin(); i!=init_list.end(); i++ ) {
				...
				gpu->get_global_memory()->write(addr,nbytes,&value,NULL,NULL);
        // assume little endian (so u8 is the first byte in u32)
				nc_bytes+=nbytes;
