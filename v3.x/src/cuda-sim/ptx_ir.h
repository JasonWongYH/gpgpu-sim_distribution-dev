...
class type_info_key {
 public:
  ...
  type_info_key(memory_space_t space_spec,
                int scalar_type_spec,
                int vector_spec,
                int alignment_spec,
                int extern_spec,
                int array_dim) {
...
class symbol_table {
 public:
  symbol_table();
  symbol_table(const char* scope_name,
  ...
  symbol* add_variable(const char* identifier,
                       const type_info* type,
                       unsigned size,
                       const char* filename,
                       unsigned line);
  void add_function(function_info* func,
                    const char* filename,
                    unsigned linenumber);
  bool add_function_decl(const char* name,
                         int entry_point,
                         function_info** func_info,
                         symbol_table** symbol_table);
  type_info* add_type(memory_space_t space_spec,
                      int scalar_type_spec,
                      int vector_spec,
                      int alignment_spec,
                      int extern_spec);
...
struct basic_block_t {
  basic_block_t(unsigned ID,
                ptx_instruction* begin,
                ptx_instruction* end,
                bool entry,
                bool ex) {
    bb_id = ID;
...
//TODO see https://stackoverflow.com/questions/35198856/half-precision-difference-between-float2half-vs-float2half-rn 
class ptx_instruction : public warp_inst_t {
 public:
  ...
  //   void set_fault(bool val){m_fault = val;}
  //   bool get_fault(){return m_fault;}
 ...
 private:
  ...
  // Rachata-page fault flag. If true, this instruction trigger a page fault
  bool m_fault;
...
class function_info {
 public:
  ...
  unsigned print_insn(unsigned pc, std::ostream& out) const;
  ...
  void add_param_name_type_size(unsigned index,
                                std::string name,
                                int type,
                                size_t size,
                                bool ptr,
  ...
  void list_param(std::ostream& out) const;
...
class arg_buffer_t {
 public:
  ...
  arg_buffer_t(const symbol* dst_sym,
               const operand_info& src_op,
