...
ptx_reg_t srcOperandModifiers(ptx_reg_t opData,
                              operand_info opInfo,
  ...
  if (op.get_double_operand_type() == 0) {
    if (((opType != BB128_TYPE) && (opType != BB64_TYPE) &&
      ...
      if (op.is_reg()) {
        ...
        const char* name = op.name().c_str();
        printf(
            "GPGPU-Sim PTX: ERROR ** get_operand_value : unknown operand type "
            "for %s\n",
            name);
        assert(0);
      }
...
void bar_sync_impl(const ptx_instruction* pI, ptx_thread_info* thread) {
  ...
  assert(b.u32 ==
         0);  // support for bar.sync a{,b}; where a != 0 not yet implemented
}
...
ptx_reg_t (*g_cvt_fn[11][11])(ptx_reg_t x,
                              unsigned from_width,
                              unsigned to_width,
                              int to_sign,
                              int rounding_mode,
...
void mad_def(const ptx_instruction* pI,
             ptx_thread_info* thread,
...
typedef unsigned (*texAddr_t)(unsigned x,
                              unsigned y,
                              unsigned mx,
                              unsigned my,
                              size_t elem_size);
float tex_linf_sampling(memory_space* mem,
